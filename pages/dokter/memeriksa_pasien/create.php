...
<script>
    $(document).ready(function() {
        // Initialize select2 on the select box for better UI
        $('#id_obat').select2();

        // Event listener for changes on the obat select box
        $('#id_obat').on('change', function() {
            var selectedValuesArray = $(this).val();  // Get selected values

            // Initialize the total cost with the basic examination fee
            var sum = 150000; 

            // If there are selected medicines
            if (selectedValuesArray) {
                for (var i = 0; i < selectedValuesArray.length; i++) {
                    ...
                    if (parts.length === 2) {
                        sum += parseFloat(parts[1]); // Add the price of the medicine
                    }
                }
            }
